...
var client  = mqtt.connect('mqtt://eclipse.usc.edu:1883');
 ...
 *******************************************************************/                                                                 
function contract_checking(field1, operator1,tmp_real_value,  tmp_threshold_value){
	var flag1=0;
	if(field1=='Temperature')
		if(operator1=='>')
			if(tmp_real_value>tmp_threshold_value)
			{flag1=1}
...
let handlers = {
  ...
  checkTx (request)  {
		...
		if (contract_checking('Temperature', '>',json_data.Temperature, 30)){
		client.on('connect', function () {
					client.subscribe('presence', function (err) {
						if (!err) {
								client.publish('alert', 'high temperature')
